...
def clear_terminal():
    # Clear the terminal based on the user's operating system
...
def save_client(client):
    with open(CLIENTS_FILE, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([client['ID'], client['Imię'], client['Nazwisko']])
...
def add_client():
    ...
    client = {'ID': client_id, 'Imię': first_name, 'Nazwisko': last_name}
    save_client(client)
